/**
 * foxhound.js — foxOS Security Diagnostic & Credential Decryption Matrix
 * 
 * Schematics:
 * ┌───────────────────────────┬───────────────────────────────────────────┬───────────────────────────┐
 * │     LEFT HUD PANEL        │              CENTER PLAYFIELD             │      RIGHT HUD PANEL      │
 * │  - Credential Fragment    │  - 2D Canvas (960 x 640)                  │  - System Status / Lives  │
 * │  - Run Statistics         │  - Dynamic Bricks & Countermeasures       │  - Real-Time Event Log    │
 * │                           │  - Paddle & Ball Physics                  │                           │
 * └───────────────────────────┴───────────────────────────────────────────┴───────────────────────────┘
 *
 * Provides:
 * 1. 3-Stage Security Breach Puzzle (HANDSHAKE -> DECRYPT -> INJECT)
 * 2. Anomaly Countermeasure System (Ghost, Glitch, Mover, and Teleport Bricks)
 * 3. 100% Theme-Reactive Phosphor Rendering (Canvas & HUD adapt live to active CRT theme)
 * 4. Acoustic Feedback via foxOS audio synthesizer
 * 5. Credential State Persistence for foxClaw unlock
 */

import { foxhoundAudio, playDiskSeek, playBell, playKeyClick, getAudioContext } from "../../core/audio.js";
...
// World / Playfield geometry
let world = null;

const deadText = "You failed to spoof the needed credentials with foxHound. Data corrupted. All progress lost. Restart foxHound? [Y/N]";
...
function getThemePalette() {
    ...
    const phosphor = computed.getPropertyValue("--phosphor").trim() || "#5bf870";
    const phosphorRgb = computed.getPropertyValue("--phosphor-rgb").trim() || "91, 248, 112";
    const tubeBase = computed.getPropertyValue("--tube-base").trim() || "#020902";
    const phosphorGlow = computed.getPropertyValue("--phosphor-glow").trim() || "0 0 8px rgba(91, 248, 112, 0.6)";

    return { phosphor, phosphorRgb, tubeBase, phosphorGlow };
}
...
async function ensureAssets() {
    if (loaded && document.getElementById("foxhound-wrap")) return;

    if (!loaded) {
        loaded = true;
        const link = document.createElement("link");
        link.rel = "stylesheet";
        link.href = "./programs/foxhound/foxhound.css";
        document.head.appendChild(link);
    }

    if (!document.getElementById("foxhound-wrap")) {
        const crt = document.getElementById("crt");
        const html = await fetch("./programs/foxhound/foxhound.html").then(r => r.text());
        crt.insertAdjacentHTML("afterbegin", html);
    }
}

// ─── Launch Entry Point ───────────────────────────────────────────────────────
export async function launchFoxhound(ctx) {
    const isRepaired = localStorage.getItem("helpRepaired") === "true";

    if (!isRepaired) {
        await ctx.type("ERROR: Security subsystem integrity compromised.\nRun 'repair' to restore file matrix.");
        return;
    }

    ctx.setInputEnabled(false);
    unlockCipher("hidden_game");
    runState = createRunState();
    await ensureAssets();

    await new Promise(resolve => {
        setTimeout(() => initGame(ctx, resolve), 50);
    });
}
...
function createRunState() {
    return {
        ...
        credentialParts: [
            ...
            createCredentialPart("L00kWhatIC4nD0"),
            createCredentialPart("CHECKSUM:Yq23Q9+7r,rE")
        ],
        log: []
    };
}
...
function showModalDialog(text) {
    let modal = document.getElementById("foxhound-modal");
    if (!modal) {
        modal = document.createElement("div");
        modal.id = "foxhound-modal";
        ...
        modal.style.border = "2px solid var(--phosphor)";
        modal.style.background = "var(--tube-base, #020902)";
        modal.style.boxShadow = "0 0 20px rgba(var(--phosphor-rgb), 0.4)";
        modal.style.color = "var(--phosphor)";
        modal.style.textAlign = "center";
        modal.style.fontFamily = "inherit";
        modal.style.fontSize = "1.35rem";
        modal.style.maxWidth = "80%";
        modal.style.whiteSpace = "pre-line";

        const centerEl = document.getElementById("foxhound-center") || document.getElementById("foxhound-wrap");
        if (centerEl) centerEl.appendChild(modal);
    }
...
function hideModalDialog() {
    const modal = document.getElementById("foxhound-modal");
...
function stopGame(onDone = () => {}) {
    ...
    const term = document.getElementById("terminal-buffer");
...
// ─── Init Game Session ────────────────────────────────────────────────────────
function initGame(terminalCtx, onDone = () => {}) {
    getAudioContext();
    ...
    document.addEventListener("keydown", async (e) => {
        ...
        if (e.key === "Escape") {
            ...
            stopGame(onDone);
            terminalCtx.clear();
            terminalCtx.setInputEnabled(true);
            terminalCtx.print("[SYSTEM] foxHound session aborted. Matrix disconnected.");
            return;
        }
        ...
        if (levelState?.gameOver) {
            ...
            if (pendingAction === "final-complete") {
                hideModalDialog();
                await handleFinalContinue(terminalCtx, onDone);
                return;
            }
            if (e.code === "KeyY" || e.key === "y" || e.key === "Y") {
                ...
                terminalCtx.clear();
                terminalCtx.setInputEnabled(true);
                terminalCtx.print("[SYSTEM] foxHound breach terminated.");
            }
            return;
        }
        ...
        if (e.key === "ArrowLeft" || e.code === "KeyA" || e.key === "a") {
            paddle.left = true;
        } else if (e.key === "ArrowRight" || e.code === "KeyD" || e.key === "d") {
            paddle.right = true;
        }
    }, { signal });
...
function resizeCanvas() {
    ...
    const fullWidth = layoutConfig.hudWidth + canvasBase.width + layoutConfig.hudWidth + layoutConfig.gap * 2;
...
function renderHud() {
    ...
    const credentialLabels = ["USER", "PASS", "CHECKSUM"];
...
function getLiveCountermeasureSummary() {
    ...
    const counts = { glitch: 0, ghost: 0, mover: 0, teleport: 0 };
...
// ─── World / Geometry Sizing ──────────────────────────────────────────────────
function computeWorld(levelDef) {
    const boardWidth = levelDef.cols * preferredBrickW + (levelDef.cols - 1) * preferredBrickPad;
    const boardHeight = levelDef.rows * preferredBrickH + (levelDef.rows - 1) * preferredBrickPad;
...
function setSpeedTier(nextTier) {
    const clamped = Math.max(0, Math.min(currentLevelDef.speedTiers.length - 1, nextTier));
    ...
    foxhoundAudio.paddleHit();
}

function resetBallProgressForAttempt() {
    levelState.rallyHits = 0;
    levelState.speedRowBoosts = { upperHalf: false, topThree: false };
...
function updateSpeedTierFromClassicRules(brickRow) {
    ...
    } else if (brickRow < Math.floor(currentLevelDef.rows / 2) && !levelState.speedRowBoosts.upperHalf) {
        levelState.speedRowBoosts.upperHalf = true;
        setSpeedTier(Math.max(ball.speedTier, 2));
    }

    if (currentLevelDef.id === 2 && currentLevelDef.speedTiers.length >= 5 && !levelState.speedRowBoosts.finalTier && getLevelClearRatio() >= 0.5) {
...
function willBallThreatenBrick(brick, lookahead = 10) {
    ...
    const cx = brick.x + brick.w / 2;
    const cy = brick.y + brick.h / 2;
    return Math.hypot(futureX - cx, futureY - cy) < 50;
}

// ─── Credential Decryption Display ────────────────────────────────────────────
...
function handleLevelClear() {
    ...
    if (runState.levelIndex === levelDefs.length - 1) {
        ...
        if (!foxhoundState.completedOnce) {
            ...
            showModalDialog(`★ SPOOF SUCCESSFUL ★\n\n${getFullCredentialString()}\n\nfoxHound may now access foxClaw.\nPress any key to continue.`);
        } else {
            showModalDialog(`★ CREDENTIAL CHECKSUM COMPLETE ★\n\nNo corruption detected.\nPress any key to continue.`);
        }
        return;
    }

    pendingAction = "next-level";
    showModalDialog(`Level cleared.\nProceed to ${levelDefs[runState.levelIndex + 1].name}? [Y/N]`);
}
...
function handleLevelFailure() {
    ...
    if (levelState.attemptsLeft <= 0) {
        ...
        showModalDialog(deadText);
        return;
    }

    pendingAction = "retry-attempt";
    pushLog(`ATTEMPT LOST :: ${levelState.attemptsLeft} ATTEMPT REMAINING ON THIS LEVEL`);
    showModalDialog(`Payload lost. Attempts remaining for this level: ${levelState.attemptsLeft}.\nContinue current breach? [Y/N]`);
}

async function handleFinalContinue(terminalCtx, onDone) {
    pendingAction = null;
    stopGame(onDone);
    terminalCtx.clear();
    terminalCtx.setInputEnabled(false);

    await terminalCtx.type([
        "Incoming Connection....",
        "Connection Established.",
        "CyberVixen > Nice. You've done it. I don't know who you are. Don't want to know. foxOS landing in your hands is all I need.",
        "CyberVixen > foxHound has spoofed the credentials for foxClaw. You're ready to begin but before that, a couple things you should know.",
        "CyberVixen > Serenity is more than ready to kill to keep foxOS under wraps. If you're discovered, you'll only have a few minutes to get the hell out of wherever you are.\nWhen that happens, ditch the net, stay away from cities, and do not touch another Serenity Industries terminal again. They will find you.",
        "CyberVixen > foxOS likely got scrambled during transfer. Sorry. Had to rush. But I've embedded 'foxClaw' directly into your terminal suite to compensate.",
        "CyberVixen > foxClaw is a rogue-kernel tactical probe. A turn-based breach protocol into Serenity's deeper sub-nets.",
        "CyberVixen > Once launched, you'll control an isolated Kernel navigating grid-based network sectors. Every move and action costs system ticks.",
        "CyberVixen > Scavenge subroutines, bypass Serenity security daemons, and descend deeper into the core. If your kernel gets dereferenced, the run is purged.",
        "CyberVixen > Type 'foxclaw' to launch the probe. Use arrow keys or Numpad to navigate the grid. Once inside, run 'help' or check your equipment.",
        "CyberVixen > You'll figure it out.",
        "CyberVixen > Remember. Stay quiet, don't get caught, and don't bite off more than you and foxOS can chew at once. Start slow. You'll be running with the pack soon enough.",
        "CyberVixen > Good luck",
        ">",
        ">",
        ">",
        "CyberVixen > You'll need it.",
        "CyberVixen > Ciao~"
    ], { speed: 14, lineDelay: 120 });
...
// ─── Theme-Reactive Canvas Rendering ──────────────────────────────────────────
function beginWorldRender() {
    ctx2d.save();
    ctx2d.scale(world.scale, world.scale);
}
...
function drawStartOverlay(palette) {
    ...
    ctx2d.save();
    ctx2d.strokeStyle = palette.phosphor;
    ctx2d.fillStyle = palette.tubeBase;
    ctx2d.lineWidth = 1.5;
    ...
    ctx2d.fillStyle = palette.phosphor;
    ctx2d.font = "18px 'VT323', monospace";
    ctx2d.textAlign = "center";
    ctx2d.textBaseline = "middle";
    ctx2d.fillText("Use A/D or L/R arrow keys to move paddle.", x + boxW / 2, y + 38);
    ctx2d.fillText("Press any key to begin payload launch.", x + boxW / 2, y + 72);

    ctx2d.restore();
}

function drawCallout(palette) {
    ...
    ctx2d.save();
    ctx2d.strokeStyle = palette.phosphor;
    ctx2d.fillStyle = `rgba(${palette.phosphorRgb}, 0.15)`;
    ctx2d.lineWidth = 1;
    ...
    ctx2d.fillStyle = palette.phosphor;
    ctx2d.font = "16px 'VT323', monospace";
    ctx2d.textAlign = "center";
    ctx2d.textBaseline = "middle";
    ctx2d.fillText(callout.text, x + boxW / 2, y + boxH / 2);

    ctx2d.restore();
}

function drawPaddle(palette) {
    const { x, y, w, h } = paddle;

    ctx2d.save();
    ctx2d.strokeStyle = palette.phosphor;
    ctx2d.fillStyle = `rgba(${palette.phosphorRgb}, 0.25)`;
    ctx2d.lineWidth = 1.5;
...
function drawBall(palette) {
    ...
    if (hasShield) {
        ctx2d.beginPath();
        ctx2d.arc(ball.x, ball.y, ball.r + 4, 0, Math.PI * 2);
        ctx2d.strokeStyle = palette.phosphor;
        ctx2d.lineWidth = 1.2;
        ctx2d.stroke();
        ctx2d.closePath();
    ...
    ctx2d.beginPath();
    ctx2d.arc(ball.x, ball.y, Math.max(2, ball.r * 0.35), 0, Math.PI * 2);
...
function drawBricks(palette) {
    ...
    for (let c = 0; c < currentLevelDef.cols; c++) {
        for (let r = 0; r < currentLevelDef.rows; r++) {
            ...
            if (b.type === "ghost") {
                drawGhostBrick(b, palette);
            } else if (b.type === "glitch") {
                drawGlitchBrick(b, palette);
            } else if (b.type === "mover") {
                drawMoverBrick(b, palette);
            } else if (b.type === "teleport") {
                drawTeleportBrick(b, palette);
            } else {
                drawNormalBrick(b, palette);
            }
        }
    }
}

function drawBrickBursts(palette) {
...
function drawNormalBrick(b, palette) {
    const { x, y, w, h } = b;

    ctx2d.save();
    ctx2d.strokeStyle = palette.phosphor;
    ctx2d.fillStyle = `rgba(${palette.phosphorRgb}, 0.2)`;
    ctx2d.lineWidth = 1;
    ...
    ctx2d.beginPath();
    ctx2d.moveTo(x + 6, y + h / 2);
    ctx2d.lineTo(x + 18, y + h / 2);
    ctx2d.moveTo(x + w - 18, y + h / 2);
    ctx2d.lineTo(x + w - 6, y + h / 2);
    ctx2d.stroke();
    ctx2d.closePath();
...
function drawMoverBrick(b, palette) {
    const { x, y, w, h } = b;

    ctx2d.save();
    ctx2d.strokeStyle = palette.phosphor;
    ctx2d.fillStyle = `rgba(${palette.phosphorRgb}, 0.35)`;
    ctx2d.lineWidth = 1;
...
function drawTeleportBrick(b, palette) {
    const { x, y, w, h } = b;
    ...
    ctx2d.textAlign = "center";
    ctx2d.textBaseline = "middle";
    ctx2d.fillText(glyphs[phase], x + w / 2, y + h / 2);

    ctx2d.restore();
}

function drawGlitchBrick(b, palette) {
    const { x, y, w, h } = b;

    ctx2d.save();
    ctx2d.strokeStyle = palette.phosphor;
    ctx2d.fillStyle = `rgba(${palette.phosphorRgb}, 0.3)`;
    ctx2d.lineWidth = 1;
    ...
    if (!scrambled) {
        ...
        ctx2d.fillText(junk[(b.row + b.col + Math.floor(performance.now() / 50)) % junk.length], x + w / 2, y + h / 2);
    }

    ctx2d.restore();
}

function drawGhostBrick(b, palette) {
    const { x, y, w, h } = b;
    ...
    ctx2d.setLineDash(corporeal ? [] : [4, 3]);
    ctx2d.lineDashOffset = corporeal ? 0 : -Math.floor(performance.now() / 50);
...
function makeInitialGlitchSlots() {
    ...
    if (slots.length === 0) slots.push(Math.floor(Math.random() * totalSlots));
...
function rerollGlitchSlots(currentSlots) {
    ...
    if (next.size === 0) next.add(Math.floor(Math.random() * totalSlots));
...
function reviveRandomNormalBrick(excludeBrick = null) {
    ...
    for (let c = 0; c < currentLevelDef.cols; c++) {
        for (let r = 0; r < currentLevelDef.rows; r++) {
            const b = bricks[c][r];
            if (b === excludeBrick || b.alive || b.special) continue;
            candidates.push(b);
        }
    }
...
function activateAnomalyWave(requested) {
    for (const [type, count] of Object.entries(requested)) {
        ...
        for (let c = 0; c < currentLevelDef.cols; c++) {
            for (let r = 0; r < currentLevelDef.rows; r++) {
                const b = bricks[c][r];
                if (!b.alive || b.type !== "normal") continue;
                candidates.push(b);
            }
        }
...
function updateBricks() {
    for (let c = 0; c < currentLevelDef.cols; c++) {
        for (let r = 0; r < currentLevelDef.rows; r++) {
            ...
            b.alive = false;
            spawnBrickBurst(b);
            foxhoundAudio.brickHit(b.row, b.special);
            ...
            // ─── Glitch Brick Auto-Collapse Win Condition Check ───
            let totalAlive = 0;
            let normalAlive = 0;
            const remainingGlitches = [];

            for (let col = 0; col < currentLevelDef.cols; col++) {
                for (let row = 0; row < currentLevelDef.rows; row++) {
                    const checkB = bricks[col][row];
                    if (checkB.alive) {
                        totalAlive++;
                        if (checkB.type === "glitch") {
                            remainingGlitches.push(checkB);
                        } else {
                            normalAlive++;
                        }
                    }
            ...
            // If only glitched bricks remain on the board, count as cleared and collapse them!
            if (normalAlive === 0 && remainingGlitches.length > 0) {
                remainingGlitches.forEach(gb => {
                    gb.alive = false;
                    spawnBrickBurst(gb);
                    score++;
                    levelState.score++;
                ...
                foxhoundAudio.glitchCollapse();
                pushLog("MATRIX DESTABILIZED :: CORRUPTED NODES COLLAPSED");
                showBanner("CORRUPTED NODES COLLAPSED :: SECTOR SECURED");
                renderHud();
                handleLevelClear();
                return;
            }
...
function updateBrickBursts() {
    for (let i = brickBursts.length - 1; i >= 0; i--) {
        ...
        p.vy += 0.03;
        p.life--;
        if (p.life <= 0) brickBursts.splice(i, 1);
    }
}
...
function loop() {
    ...
    // Fill background with active cathode tube base
    ctx2d.fillStyle = palette.tubeBase;
    ...
    if (!levelState?.gameOver) {
        ...
        beginWorldRender();
        drawBricks(palette);
        drawBrickBursts(palette);
        drawPaddle(palette);
        drawBall(palette);
        drawCallout(palette);
        drawStartOverlay(palette);
        endWorldRender();
